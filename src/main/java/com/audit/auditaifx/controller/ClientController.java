...
public class ClientController {
    ...
    private com.example.mindjavafx.controller.UserDashboardController dashboardController;

    public void setDashboardController(com.example.mindjavafx.controller.UserDashboardController db) {
        this.dashboardController = db;
    }
    ...
    @FXML
    private javafx.scene.layout.VBox lblNoSelection;
    @FXML
    ...
    @FXML
    private TableColumn<RapportAudit, RapportAudit> colActionsRapport;
    ...
    public void initialize() {
        ...
        colStatut.setCellValueFactory(cellData -> {
            String rawScore = cellData.getValue().getScoreAudit();
            if (rawScore == null || rawScore.isEmpty()) return new javafx.beans.property.SimpleStringProperty("N/A");
            try {
                if (rawScore.startsWith("{")) {
                    com.google.gson.JsonObject data = com.google.gson.JsonParser.parseString(rawScore).getAsJsonObject();
                    double val = data.get("global").getAsDouble();
                    return new javafx.beans.property.SimpleStringProperty(String.format("%.1f", val).replace(".", ",") + "/10");
                }
                if (!rawScore.contains("/10")) return new javafx.beans.property.SimpleStringProperty(rawScore + "/10");
                return new javafx.beans.property.SimpleStringProperty(rawScore);
            } catch (Exception e) {
                return new javafx.beans.property.SimpleStringProperty(rawScore);
            }
        });

        // Ensure actions column is visible and populated
        colActionsRapport.setCellValueFactory(param -> new javafx.beans.property.ReadOnlyObjectWrapper<>(param.getValue()));
    ...
    private void ajouterBoutonsRapportsTable() {
        colActionsRapport.setCellFactory(param -> new TableCell<RapportAudit, RapportAudit>() {
...

            {
                ...
                btnVoir.setOnAction(e -> {
                    RapportAudit r = (RapportAudit) getTableRow().getItem();
                    if (r != null) showReportDetails(r);
                });
                btnEdit.setOnAction(e -> {
                    RapportAudit r = (RapportAudit) getTableRow().getItem();
                    if (r != null) ouvrirFormulaireRapport(r);
                });
                btnDel.setOnAction(e -> {
                    RapportAudit r = (RapportAudit) getTableRow().getItem();
                    if (r != null) supprimerRapportManual(r);
                });
            }
    ...
    private void rafraichirStats() {
        ...
        if (lblStatTotalRapports != null) lblStatTotalRapports.setText(String.valueOf(totalRapports));
        if (lblStatTotalReco != null) lblStatTotalReco.setText(String.valueOf(totalReco));
        if (lblStatRecoResolues != null) lblStatRecoResolues.setText(String.valueOf(recoResolues));
    }
    ...
    private void rafraichirListe() {
        if (currentUser != null) {
            allReports = service.getTous().filtered(r -> 
    ...
    private void handleGenererRapportDepuisScan() {
        ...
        nouveau.setEntiteAuditee(currentUser != null ? currentUser.getNom() : "Entité Détectée");
    ...
    private void showReportDetails(RapportAudit report) {
        ...
        lblAuditeur.setText("📁 Catégorie : " + (report.getAuditeur() != null ? report.getAuditeur() : "Non classé"));
        lblDate.setText("📅 " + (report.getDateCreation() != null ? report.getDateCreation().toString() : "Date inconnue"));
        
        // Handle score display (JSON or raw string)
        String scoreDisplay = "N/A";
        if (report.getScoreAudit() != null && !report.getScoreAudit().isEmpty()) {
            try {
                if (report.getScoreAudit().startsWith("{")) {
                    com.google.gson.JsonObject data = com.google.gson.JsonParser.parseString(report.getScoreAudit()).getAsJsonObject();
                    scoreDisplay = String.format("%.1f", data.get("global").getAsDouble());
                } else {
                    scoreDisplay = report.getScoreAudit();
                }
            } catch (Exception e) {
                scoreDisplay = report.getScoreAudit();
            }
        }
        lblStatut.setText("📊 Score : " + scoreDisplay + "/10");
        ...
        if (report.getRisques().isEmpty()) {
            Label noRisque = new Label("✅ Aucun risque détecté pour ce rapport.");
            noRisque.setStyle("-fx-text-fill: #27ae60; -fx-font-weight: bold; -fx-padding: 10; -fx-background-color: #f0fff4; -fx-background-radius: 5;");
            risquesContainer.getChildren().add(noRisque);
        } else {
    ...
    private void afficherBadgeScore(String scoreStr) {
        try {
            ...
            final String finalScoreStr = scoreStr;
            lblScoreBadge.setOnMouseClicked(e -> ouvrirModalScore(finalScoreStr));
            lblScoreBadge.setCursor(javafx.scene.Cursor.HAND);
        } catch (Exception e) { 
            System.err.println("Erreur affichage badge score: " + e.getMessage());
            lblScoreBadge.setVisible(false);
        }
    }
    ...
    private void ouvrirModalScore(String json) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/rapports_reco/score-modal.fxml"));
    ...
    private void ouvrirFormulaireRapport(RapportAudit rapport) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/rapports_reco/rapport-form-view.fxml"));
            Parent root = loader.load();
    ...
    @FXML
    public void sendToChatbotForRecommendations() {
        if (rapportSelectionne != null && dashboardController != null) {
            dashboardController.openChatbotWithReport(rapportSelectionne);
        }
    }
    ...
    public void switchAdmin() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/rapports_reco/main-view.fxml"));
            Parent root = loader.load();
