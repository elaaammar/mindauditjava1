...
public class ProfessionalDashboardController {
    ...
    public void setAuthService(AuthenticationService authService) {
        ...
        if (currentUser != null) {
            ...
            // Rendre la cloche rouge pour les alertes de sécurité
            notificationButton.setStyle("-fx-background-color: #e74c3c; -fx-text-fill: white; -fx-background-radius: 20px; -fx-padding: 5px; -fx-font-size: 20px; -fx-cursor: hand;");
            notificationBadge.setVisible(true);
            notificationBadge.setText("+1");
            notificationBadge.setStyle("-fx-background-color: #c0392b; -fx-text-fill: white; -fx-background-radius: 10px; -fx-padding: 2px 6px; -fx-font-size: 10px; -fx-font-weight: bold;");
    ...
    public void updateNotificationBadge() {
        if (currentUser != null && notificationService != null) {
            ...
            if (unreadCount > 0) {
                ...
                notificationBadge.setVisible(true);
                // Rendre l'alerte rouge TRÈS visible
                ...
                notificationButton.setStyle("-fx-background-color: #e74c3c; -fx-text-fill: white; -fx-background-radius: 20px; -fx-cursor: hand; -fx-font-size: 20px;");
            } else {
    ...
    private void showNotificationDropdown() {
        ...
        if (notifs.isEmpty()) {
            content.append("🛡️ Alerte Sécurité : Un e-mail de notification a été envoyé à eleammar21@gmail.com pour confirmer votre connexion.");
        } else {
    ...
    private void handleSearch() {
        ...
        try {
            java.util.List<User> results = userService.searchByName(query);
            loadSection("user-search-results.fxml", (controller) -> {
                if (controller instanceof UserSearchResultsController) {
                    ((UserSearchResultsController) controller).setResults(query, results);
                }
            });
            highlightButton(null); // Deselect all menu buttons
        } catch (Exception e) {
            e.printStackTrace();
            showErrorDialog("Erreur de recherche", "Impossible d'effectuer la recherche : " + e.getMessage());
        }
    }
